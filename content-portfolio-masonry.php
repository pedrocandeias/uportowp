...
<article <?php post_class( $post_class ); ?>>
	...
	<div class="portfolio-title<?php echo 'widewidth' === $porto_layout ? ' container m-t-lg' : ''; ?>">
		<div class="row">
			...
			<div class="portfolio-nav col-lg-1">
				<?php
				previous_post_link( '%link', '<div data-tooltip title="' . esc_attr__( 'Previous', 'porto' ) . '" class="portfolio-nav-prev"><i class="fa"></i></div>' );
				next_post_link( '%link', '<div data-tooltip title="' . esc_attr__( 'Next', 'porto' ) . '" class="portfolio-nav-next"><i class="fa"></i></div>' );
				?>
			</div>
		</div>
	</div>
	...
	<div class="row<?php echo 'widewidth' === $porto_layout ? ' m-t-lg m-r m-l' : ''; ?>">
		<?php
											...
											<span class="thumb-info-wrapper">
												...
												<?php if ( $porto_settings['portfolio-zoom'] ) : ?>
													...
													<a href="<?php echo esc_url( $attachment['src'] ); ?>" class="lightbox-portfolio" title="<?php echo ( isset( $attachment['caption'] ) ? $attachment['caption'] : '' ); ?>">
														<span data-title="<?php echo ( isset( $attachment['caption'] ) ? $attachment['caption'] : '' ); ?>" class="thumb-info-action-icon thumb-info-action-icon-light"><i class="fa fa-search-plus"></i></span>
													</a>
												</span>
												<?php endif ?>
											</span>
		...
		<?php endif; ?>
			...
			<?php if ( ! $attachment && $porto_settings['share-enable'] && 'no' !== $share && ( 'yes' === $share || ( 'yes' !== $share && $porto_settings['portfolio-share'] ) ) ) : ?>
				<hr class="tall">
				<div class="share-links-block">
					<h5><?php esc_html_e( 'Share', 'porto' ); ?></h5>
					<?php get_template_part( 'share' ); ?>
