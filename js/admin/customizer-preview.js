jQuery(document).ready(function($) {
        ...
        wp.customize.selectiveRefresh.bind('partial-content-rendered', function (placement) {
            ...
            if (placement.partial.id == 'header-wrapper' || placement.partial.id == 'header') {
                if ('side' == wp.customize.instance('porto_header_builder[type]').get()) {
                    $('.header-wrapper').addClass('header-side-nav');
                    $('.page-wrapper').addClass('side-nav');
                    $(document.body).addClass('body-side');
                } else {
                    $('.header-wrapper').removeClass('header-side-nav');
    ...
    wp.customize('porto_settings[mainmenu-popup-heading-color]', function(e) {
        e.bind(function(value) {
            appendStyle('porto_settings[mainmenu-popup-heading-color]', '#header .main-menu .wide .popup li.sub > a, .header-side-nav .sidebar-menu .wide .popup > .inner > ul.sub-menu > li.menu-item > a, .toggle-menu-wrap .sidebar-menu .wide .popup > .inner > ul.sub-menu > li.menu-item > a { color: ' + value + ' }');
        });
    });
    ...
    wp.customize('porto_settings[product-related]', function(e) {
        e.bind(function(value) {
            if ('0' == value) {
                $('.single-product .porto-related-products').remove();
            } else if (!$('.single-product .porto-related-products').length) {
                if ('left_sidebar' == window.parent.wp.customize.instance('porto_settings[product-single-content-layout]').get()) {
                    $('.single-product .site-main > .product').append('<div class="porto-related-products d-none"></div>');
                } else {
                    $('.single-product div#main').append('<div class="porto-related-products d-none"></div>');
                }
            }
        });
    });
