...
function porto_ajax_portfolio_like() {
	if ( isset( $_POST['portfolio_id'] ) ) {
		...
		echo '<span class="portfolio-liked linked" title="' . esc_attr__( 'Already Liked', 'porto' ) . '" data-tooltip><i class="fa fa-heart"></i>' . $like_count . '</span>';
	}
...
function porto_portfolio_like() {
	...
	if ( $like_count && isset( $_COOKIE[ 'porto_like_' . $portfolio_id ] ) ) {
		$output = '<span class="portfolio-liked linked" title="' . esc_attr__( 'Already liked', 'porto' ) . '" data-tooltip><i class="fa fa-heart"></i>' . $like_count . '</span>';
	} else {
		$output = '<span class="portfolio-like" title="' . esc_attr__( 'Like', 'porto' ) . '" data-tooltip data-id="' . $portfolio_id . '"><i class="fa fa-heart"></i>' . ( $like_count ? $like_count : '0' ) . '</span>';
	}
